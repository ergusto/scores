...
import { useNewGameState, useNewGameStepProgress, type Step } from "@/ui/state/newGame";
      ...
      <nav className="md:hidden px-2 flex" aria-label="Breadcrumb">
        <ol className="inline-flex items-center space-x-1">
          {steps.map(step => {
            ...
            let icon = <svg aria-hidden="true" className="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fillRule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clipRule="evenodd"></path></svg>;
            ...
            return (
              <li key={`new-game-index-${step.number}`} className="inline-flex items-center" onClick={() => actions.setCurrentStep(step.number)}>
                <button disabled={!canContinue} className="inline-flex items-center text-sm font-medium text-gray-700 cursor-pointer disabled:opacity-50">
                  {icon}
                  Step {step.number}
                </button>
              </li>
